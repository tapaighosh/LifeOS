...
export const challengeAcceptSchema = z.object({
  ...
  pillar: z.enum(['money', 'soul', 'curiosity'], {
    ...
    invalid_type_error: 'Pillar must be money, soul, or curiosity',
  }),
  frequency: z
    .enum(['daily', 'alternate', '3x_week', 'weekly'], {
      invalid_type_error: 'Frequency must be one of: daily, alternate, 3x_week, weekly',
    })
    .optional(),
});
  ...
  .object({
    status: z
      .enum(['dropped', 'paused', 'active'], {
        invalid_type_error: 'Status must be one of: dropped, paused, active',
      })
      .optional(),
    notes: z
      .string()
